...
var ReactBingmaps = function (_Component) {
	...
	_createClass(ReactBingmaps, [{
		...
		value: function componentWillReceiveProps(nextProps) {
			...
			if (this.props.lightweightInfoboxesWithPushPins !== nextProps.lightweightInfoboxesWithPushPins) {
				this.setLightweightInfoboxesWithPushPins(nextProps.lightweightInfoboxesWithPushPins, "lightweightInfoboxesWithPushPins", mapReference);
			}
			...
			if (this.props.irregularPolygons !== nextProps.irregularPolygons) {
				this.setIrregularPolygons(nextProps.irregularPolygons, mapReference);
			}
		...
		value: function reactBingmaps(props, Microsoft) {
			var bingmapKey = props.bingmapKey,
			    ...
			    lightweightInfoboxesWithPushPins = props.lightweightInfoboxesWithPushPins,
			...
			if (bingmapKey && Microsoft) {
				...
				if (!map[mapReference]) {
					var m = new Microsoft.Maps.Map(mapReference, {
				...
				this.setLightweightInfoboxesWithPushPins(lightweightInfoboxesWithPushPins, mapReference)
				...
				this.setIrregularPolygons(irregularPolygons, mapReference);
		...
		value: function createRegularPolygons(regularPolygons, mapReference) {
			...
			key: 'setLightweightInfoboxesWithPushPins',
			value: function setLightweightInfoboxesWithPushPins(lightweightInfoboxesWithPushPins, infoboxCreateType, mapReference) {
				if (map[mapReference] && lightweightInfoboxesWithPushPins) {
					//Remove existing Infoboxes
					if (infobox[infoboxCreateType]){
						infobox[infoboxCreateType].setMap(null);
					}

					//Remove existing Pushpins
					infobox[infoboxCreateType] = new Microsoft.Maps.Infobox(new Microsoft.Maps.Location(lightweightInfoboxesWithPushPins[0].location[0], lightweightInfoboxesWithPushPins[0].location[1]), {visible:true})
					infobox[infoboxCreateType].optionList = []
					infobox[infoboxCreateType].setMap(map[mapReference])
					//Add Infoboxes with Pushpins
					for (var infoboxWithPushPinIndex = 0; infoboxWithPushPinIndex < lightweightInfoboxesWithPushPins.length; infoboxWithPushPinIndex++) {
						//Set Infobox Option
						var infoboxOption = lightweightInfoboxesWithPushPins[infoboxWithPushPinIndex].infoboxOption ? lightweightInfoboxesWithPushPins[infoboxWithPushPinIndex].infoboxOption : null;
						...
						//Set Pushpin Option
						var pushPinOption = lightweightInfoboxesWithPushPins[infoboxWithPushPinIndex].pushPinOption ? lightweightInfoboxesWithPushPins[infoboxWithPushPinIndex].pushPinOption : null;
						...
						//Set Infobox
						infobox[infoboxCreateType].optionList.push(infoboxOption);
					}
					var locations = lightweightInfoboxesWithPushPins.map(function (p) {
						return new Microsoft.Maps.Location(p.location[0], p.location[1]);
					})
					...
					Microsoft.Maps.loadModule('CanvasOverlayModule', function () {
						var overlay = new CanvasOverlay(function (canvas) {
							...
							var points = map[mapReference].tryLocationToPixel(locations, Microsoft.Maps.PixelReference.control);
							//Get the context of the main canvas.
							var ctx = canvas.getContext("2d");

							//Buffer map dimensions to account for radius of points.
							var mapWidth = map[mapReference].getWidth() + 7;
							var mapHeight = map[mapReference].getHeight() + 7;
							//Draw the off screen canvas for each location.
							for (var i = 0, len = points.length; i < len; i++) {
								var option = lightweightInfoboxesWithPushPins[i].pushPinOption;
								ctx.fillStyle = option.color ? option.color : "red";
								ctx.lineWidth = option.strokeThickness ? option.strokeThickness : 0;
								ctx.strokeStyle = option.strokeColor ? option.strokeColor : 'transparent';
						...
						Microsoft.Maps.Events.addHandler(map[mapReference], 'mousemove', function (e) {
							...
							if (selectedIdx >= 0) {
								//var infobox = infobox[infoboxCreateType];
								infobox[infoboxCreateType].setLocation(locations[selectedIdx]);
								var selectedOption = infobox[infoboxCreateType].optionList[selectedIdx];
								selectedOption.showCloseButton = false;
								selectedOption.visible = true;
								infobox[infoboxCreateType].setOptions(selectedOption)
							}
						});
						map[mapReference].layers.insert(overlay);
					});
				}
			}
		}, {
		...
		key: 'setIrregularPolygons',
		value: function setIrregularPolygons(irregularPolygons, mapReference) {
...
ReactBingmaps.propTypes = {
	...
	lightweightInfoboxesWithPushPins: _propTypes2.default.arrayOf(_propTypes2.default.shape({
		location: _propTypes2.default.arrayOf(_propTypes2.default.number),
		addHandler: _propTypes2.default.string,
		infoboxOption: _propTypes2.default.object,
		pushPinOption: _propTypes2.default.object,
	...
	irregularPolygons: _propTypes2.default.arrayOf(_propTypes2.default.shape({
...
ReactBingmaps.defaultProps = {
	...
	lightweightInfoboxesWithPushPins: undefined,
	zoom: undefined,
